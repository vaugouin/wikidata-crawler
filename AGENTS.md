...
- **Two of those four counters are a floor, not a bug.** `item_cache` refuses to write a core entity into `ITEM`, and `f_getwikidatalabel` (tmdb-movie-preprocess) reads `ITEM` only, on purpose (TMDB-MOVIE-PREPROCESS-036: widening it wrote film titles into `AWARD_NAME_FR`). So a Q-id V2 already holds as a movie, serie or person keeps falling back to V1 whatever we seed. `missing` is the other floor: Q-ids deleted or merged in Wikidata since a SPARQL crawler recorded them. Both go to WIKIDATA-CRAWLER-022 as acted residual loss.

Cost: the extension adds roughly 250 k full JSON parses to a pass that already streams 120 M entities (not measurable), about two claims per newly cached item, and it grows `T_WC_WIKIDATA_ITEM` from ~702 k by at most a third. Off switch: `V1_BACKFILL_SEED=0`. Floor guard: the step fails loudly under `V1_BACKFILL_MIN_IDS` (100 000 by default) rather than running 23 h having seeded nothing.
