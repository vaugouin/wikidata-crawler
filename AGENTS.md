...
- `build_v1_backfill_seed.py` (WIKIDATA-CRAWLER-023) : builds the V1 backfill seed, the list of item ids `item_cache` must cache on top of what pass2 referenced, read from `T_WC_WIKIDATA_ITEM_V1`. Called by step 106, and runnable alone: `--report` ventilates the target set without writing anything.
...
- `apply_to_live_db.sql` — idempotent additive DDL applied automatically by the orchestrator at steps 108 and 110. Creates the SEASON/EPISODE/CHARACTER target tables and the **full** `STG_T_WC_WIKIDATA_*` staging set with `CREATE TABLE IF NOT EXISTS`, then widens `YEAR_VALUE` to BIGINT. This is what keeps a long-lived DB in sync without a fresh-database rebuild, and what makes a hand-dropped staging table a non-event instead of a crash after days of ETL. Its staging definitions duplicate `02_staging_and_triggers.sql` (which stays canonical, and is the only file that creates the triggers) — change both together. It must stay `DELIMITER`-free: the orchestrator executes it through pymysql by splitting on `;` (full-line `--` comments are dropped first, so a `;` inside one is harmless). Since 2026-09-20 it also carries `ALTER TABLE ... ADD COLUMN IF NOT EXISTS`, the 14 statements that add `ALIASES_JSON` to the seven target and seven staging entity tables: a new **column** cannot reach a live database any other way, since `CREATE TABLE IF NOT EXISTS` does nothing at all to a table that already exists. Adding a column to an entity row means editing four places in one go, `01_create_schema.sql`, `02_staging_and_triggers.sql`, and both DDL sets here.
...
| 106 | run ETL item_cache | Rebuilds the V1 backfill seed from the database first (see below), then the item-cache pass: emit `T_WC_WIKIDATA_ITEM` rows for referenced items and extra `T_WC_WIKIDATA_PERSON` rows for referenced (no-IMDb) persons, plus their `CACHED_ENTITY_PROPERTIES` claims (P31, P279, P345, P569, P570, P577). Requires pass1 + pass2 outputs **and the dump**. |
...
**The V1 backfill seed is rebuilt at every run, since 2026-09-19 (WIKIDATA-CRAWLER-023).** `item_cache` only emits a `T_WC_WIKIDATA_ITEM` row for an entity present in its referenced-item filter, so the entities that only V1 knows (a row in `T_WC_WIKIDATA_ITEM_V1`, none in `T_WC_WIKIDATA_ITEM`, 36,2 % of the French labels on screen at 2026-09-19) could never exist in V2, and deleting the V1 tables would empty them from the screen. Step 106 now reads `T_WC_WIKIDATA_ITEM_V1` first, writes `/shared/seed/v1_backfill_item_ids.txt`, and hands it to the pass as a second, separate input (`extra_item_ids_path`). Four things to know before touching it:

- **The seed is anchored on V1 alone, never on the difference "in V1, absent from V2".** That difference self-erases: after one successful import it returns zero, the seed empties, those items drop out of the filter, and step 114 deletes their facts at the next run, the whole thing under a run reporting SUCCESS. The difference is the *measure* (`doc/sql/wikidata-v1-backfill-target-set.sql`, or `./wikidata-crawler.sh --v1-backfill-report`), the V1 anchor is the *seed*. Do not merge the two.
- **What weekly re-injection actually protects is not the labels.** Entity tables carry no `IMPORT_BATCH_ID` and are never pruned, so an imported `ITEM` row survives forever and the label would not fall back to V1 next week. What re-injection keeps alive is the `CACHED_ENTITY_PROPERTIES` facts of those items, which do carry a batch id and which step 114 deletes as soon as they are not re-emitted: without it, every run would recreate the exact symptom of WIKIDATA-CRAWLER-020 (a label with not one fact). Plus label freshness, and reproducibility after a `04_reset_for_full_rerun.sql`.
- **pass2's `referenced_item_ids.txt` is never modified.** The seed is a separate file so that a later reader can still tell what this run's statements actually referenced. Step 107 writes the ventilation (`seeded` = `emitted` + `diverted` to PERSON + `skippedcore` + `missing`) to `T_WC_SERVER_VARIABLE` under `strwikidatacrawlerv1backfill*`.
- **Two of those four counters are a floor, and only one of them is a loss.** `item_cache` refuses to write a core entity into `ITEM`, and `f_getwikidatalabel` (tmdb-movie-preprocess) reads `ITEM` only, on purpose (TMDB-MOVIE-PREPROCESS-036: widening it wrote film titles into `AWARD_NAME_FR`). Measured 2026-09-19: **81 456 entities**, 12,5 % of the French V1 rows, sit there. Their label is **not lost**, V2 holds it in `SERIE` / `EPISODE` / `CHARACTER` / `SEASON`; it is the reader that does not look, so this is a tmdb-movie-preprocess ticket and not a residual loss for -022. `missing` is the real loss: Q-ids deleted or merged in Wikidata since a SPARQL crawler recorded them, count unknown until the run.

Measured target, 2026-09-19 (`doc/sql/wikidata-v1-backfill-target-set-20260919.txt`): 696 660 ids seeded, of which **167 928 are the importable target** (25,8 % of the French V1 rows), 402 312 already in `ITEM`, 81 456 the floor above. The ticket's "~250 k" was a third too high: it counted the floor as importable. Cost: 167 928 more full JSON parses on a pass that already streams 120 M entities (not measurable), about two claims per newly cached item, and `T_WC_WIKIDATA_ITEM` grows from 710 556 to at most 878 484 (+23,6 %). Expected effect on `test-017-repli-v1-taux.sql`: 36,2 % down to about **11,9 %** plus whatever `missing` turns out to be. Off switch: `V1_BACKFILL_SEED=0`. Floor guard: the step fails loudly under `V1_BACKFILL_MIN_IDS` (100 000 by default) rather than running 23 h having seeded nothing.
...
**The entity rows carry `ALIASES_JSON` since 2026-09-20 (WIKIDATA-CRAWLER-025).** `extract_aliases` reads `doc["aliases"]` next to the labels and yields `{lang: [alias, ...]}`, all languages, dropping a language whose list holds nothing usable so that an alias-less entity gives `{}` like a label-less one. It lands in the two `base_row` dicts of `emit_entity`, pass2 and item_cache, which are the only two places in the repo where an entity row is built, so those two lines cover all seven entity tables. This reopens WIKIDATA-CRAWLER-016, closed "sans objet" in August: the July search looked for a consumer that *searches by* alias and found none, correctly, but the live consumers **display** aliases, and one of them (`T_WC_T2S_ITEM`, rebuilt in full at every pass of `tmdb-movie-preprocess` process 40) re-reads `ITEM_V1.ALIASES` on every run, so it would fall empty at the first pass after the V1 cut, not merely go stale.

Three things to know before touching it. **Nothing is needed in `load_staging_jsonl.py`, and that is a trap about order, not a convenience**: the loader derives its columns from the keys of the NDJSON's first line and JSON-serializes dicts and lists, so the new key travels on its own *provided the staging column exists*; otherwise step 108 dies on an unknown column after a pass that has just run for a day. DDL first, emission second. **The shape is a blob on the entity table, not a table of its own**, deliberately: same grain as `LABELS_JSON`, same read (`JSON_UNQUOTE(JSON_EXTRACT(..., '$.fr'))`), no join. The alternative, `T_WC_WIKIDATA_ALIAS(ID_WIKIDATA, LANG, ALIAS)`, has exactly one serious argument, it alone makes searching *by* alias indexable, and the day entity resolution wants that, it derives from the blob with an `INSERT ... SELECT` rather than another dump replay. **All languages, and the volume is the one open question**: one label per language but n aliases per language over ~2.5 M entity rows, and nobody has measured what that weighs. `doc/sql/wikidata-v2-025-acceptance.sql` §F3 measures it; the figure that decides whether a language filter is warranted is not the alias count but `MO_FR_EN / MO_TOTAL`. Put the filter on the measurement, not ahead of it. Baseline taken 2026-09-20, before the run (`doc/sql/wikidata-v2-025-acceptance-20260920.txt`): the seven target tables weigh **4 653 MB** together, `ITEM` 2 367 and `PERSON` 1 404 making three quarters of it. Same file records what the alias migration cannot reach for persons: of 172 179 `PERSON_V1` rows with a non-empty `ALIASES`, 20 917 (12,1 %) have no row in `T_WC_WIKIDATA_PERSON` at all, which is entity coverage (-023) and not an alias question.

Cost and window: entity rows are emitted only by pass2 (step 104) and item_cache (step 106), so the column fills only after two full reads of the 102 GB dump. There is no reason to pay that separately, the whole pipeline runs at every new dump anyway. Shipping the code and rebuilding the image **before** `run-if-new-dump.sh` fires makes the marginal cost zero; shipping after it means waiting a week or replaying 104 to 111 for nothing. Same window as WIKIDATA-CRAWLER-023. Entity tables carry no `IMPORT_BATCH_ID` and are never pruned, so the column fills by in-place update and nothing leaves the screen meanwhile.
...
- Entity tables: `T_WC_WIKIDATA_MOVIE`, `T_WC_WIKIDATA_SERIE`, `T_WC_WIKIDATA_PERSON`, `T_WC_WIKIDATA_ITEM` (referenced-item cache only — not a mirror of all Wikidata items), plus `_SEASON`, `_EPISODE`, `_CHARACTER`. All seven carry the same localized-text block: `LABEL_EN`, `DESCRIPTION_EN`, `LABELS_JSON`, `DESCRIPTIONS_JSON`, `ALIASES_JSON`.
...
- V1 backfill seed (WIKIDATA-CRAWLER-023): `V1_BACKFILL_SEED` (default on, `0` disables the extension), `V1_BACKFILL_MIN_IDS` (default 100 000, the floor under which step 106 refuses to start). `EXTRA_ITEM_IDS` is the path form, read only by a standalone `wikidata_dump_etl.py` / `run_etl.sh` invocation; the orchestrator passes the path directly.
